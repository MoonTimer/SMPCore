...
public class HistoryCommand extends BaseCommand {
    ...
    public HistoryCommand(Plugin plugin, MessageService messages, PunishmentService punishments) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) {
            sendUsage(sender, "/history <player> [page]");
            return true;
        }
        int page = 1;
        if (args.length > 1) {
        ...
        int pageSize = 10;
        punishments.getHistory(target.getUniqueId(), finalPage, pageSize, (records, totalPages) -> {
            if (records.isEmpty()) {
                messages.send(sender, "moderation.no-history");
                return;
            }
            messages.send(sender, "moderation.history-header", Map.of(
                    "target", args[0],
