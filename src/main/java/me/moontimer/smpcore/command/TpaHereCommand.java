...
public class TpaHereCommand extends BaseCommand {
    ...
    public TpaHereCommand(Plugin plugin, MessageService messages, TpaService tpaService, CooldownManager cooldowns,
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!checkPermission(sender, "smpcore.tpa.here")) {
            return true;
        }
        if (args.length < 1) {
            sendUsage(sender, "/tpahere <player>");
            return true;
        }
        ...
        if (target.getUniqueId().equals(player.getUniqueId())) {
            messages.send(player, "errors.invalid-args", Map.of("usage", "/tpahere <player>"));
            return true;
        }
        ...
        tpaService.sendRequest(player, target, TpaService.TpaType.TPAHERE);
