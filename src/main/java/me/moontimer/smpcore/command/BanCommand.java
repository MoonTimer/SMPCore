...
public class BanCommand extends BaseCommand {
    ...
    public BanCommand(Plugin plugin, MessageService messages, PunishmentService punishments) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!checkPermission(sender, "smpcore.mod.ban")) {
            return true;
        }
        if (args.length < 1) {
            sendUsage(sender, "/ban <player> [reason] [-s]");
            return true;
        }
        ...
        punishments.createPunishment(PunishmentType.BAN, target, ip,
                ...
                sender.getName(), reason, null, id -> {
                    String message = messages.format("moderation.ban", Map.of(
                    ...
                    if (online != null) {
                        online.kickPlayer(punishments.buildBanScreen(finalReason, null, id));
                    }
                });
        return true;
    }
