...
public class TempMuteCommand extends BaseCommand {
    ...
    public TempMuteCommand(Plugin plugin, MessageService messages, PunishmentService punishments) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!checkPermission(sender, "smpcore.mod.tempmute")) {
            return true;
        }
        if (args.length < 2) {
            sendUsage(sender, "/tempmute <player> <duration> [reason] [-s]");
            return true;
        }
        ...
        punishments.createPunishment(PunishmentType.MUTE, target, ip,
                ...
                sender.getName(), reason, expiresAt, id -> {
                    ...
                    String message = messages.format("moderation.tempmute", Map.of(
